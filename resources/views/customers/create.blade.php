<x-app-layout>
    <div class="max-w-2xl mx-auto glass-panel p-6 space-y-6">
        ...
        <div class="border-b border-white/5 pb-4">
            <h2 class="text-lg font-bold text-white"><i class="fa-solid fa-user-plus ml-2 text-[#D41414]"></i>إضافة عميل جديد</h2>
            <p class="text-xs text-gray-500 mt-1">تعبئة بيانات العميل لربطه بعمليات البيع وتسجيل تذاكر الصيانة.</p>
        </div>
        ...
        <form method="POST" action="{{ route('customers.store') }}" class="space-y-4">
            ...
            <div class="space-y-1">
                ...
                >
                    ...
                    @foreach($branches as $b)
                        <option value="{{ $b->id }}" {{ old('branch_id', auth()->user()->branch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="pt-4 border-t border-white/5 flex justify-end space-x-3 space-x-reverse">
                ...
                <button type="submit" class="px-5 py-2 bg-[#D41414] hover:bg-[#A30F0F] text-white rounded-lg text-xs font-bold transition shadow-lg glow-primary">
                    حفظ العميل <i class="fa-solid fa-floppy-disk mr-1"></i>
                </button>
            </div>
